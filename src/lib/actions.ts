...
import {
  optimizePost as optimizePostFlow,
  type OptimizePostInput,
} from "@/ai/flows/optimize-student-center-post";
...
export async function optimizePost(input: OptimizePostInput) {
  try {
    const result = await optimizePostFlow(input);
    return { success: true, data: result };
  } catch (error) {
    console.error("AI Optimization Error:", error);
    if (error instanceof Error) {
        return { success: false, error: `Ha ocurrido un error con la IA: ${error.message}` };
    }
    return { success: false, error: "Ha ocurrido un error desconocido durante la optimización." };
  }
}
